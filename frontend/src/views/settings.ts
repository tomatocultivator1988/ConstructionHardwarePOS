...
export async function showShiftPreview(id: string) {
  try {
    ...
    showModal(`<div class="shift-preview"><div class="receipt-preview-heading"><div><span class="help-eyebrow">Shift Report Preview</span><h3>${esc(shift.username || 'Cashier Shift')}</h3></div><button class="help-close" onclick="closeModal()" aria-label="Close">×</button></div><div class="shift-preview-paper"><div class="shift-paper-header"><strong>JEG ENTERPRISES</strong><span>CASHIER SHIFT REPORT</span></div><dl><dt>Cashier</dt><dd>${esc(shift.username || '—')}</dd><dt>Opened</dt><dd>${esc(fmtDate(shift.opened_at))}</dd><dt>Closed</dt><dd>${esc(fmtDate(shift.closed_at))}</dd></dl><div class="shift-paper-section">CASH DRAWER</div><dl><dt>Opening Cash</dt><dd>${fmtPeso(shift.opening_cash)}</dd><dt>Cash Sales</dt><dd>${fmtPeso(shift.cash_sales)}</dd><dt>Cash Refunds</dt><dd>${fmtPeso(shift.cash_refunds)}</dd><dt>Drawer Adjustments</dt><dd>${fmtPeso(shift.drawer_events)}</dd></dl><div class="shift-paper-section">RECONCILIATION</div><dl><dt>Expected Cash</dt><dd>${fmtPeso(shift.expected_cash)}</dd><dt>Counted Cash</dt><dd>${fmtPeso(shift.closing_cash)}</dd><dt>Variance</dt><dd class="${variance === 0 ? 'positive' : variance > 0 ? 'warning-value' : 'negative'}">${variance >= 0 ? '+' : '−'}${fmtPeso(Math.abs(variance))}</dd></dl><p><strong>Notes:</strong> ${esc(shift.notes || '—')}</p><div class="shift-paper-footer">— End of Shift —</div></div><div class="modal-actions"><button class="btn" onclick="closeModal()">Close</button><button class="btn btn-primary" onclick="printShift('${id}')">Print Thermal</button></div></div>`, 'shift-preview-modal');

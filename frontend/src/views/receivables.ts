...
export async function renderReceivables(): Promise<string> {
  ...
  return `<div class="page-header"><div><div class="page-kicker">Credit sales</div><h2>Receivables</h2><p class="page-subtitle">Track customer balances and credit sales in one place.</p></div></div>
    ...
    <section class="dashboard-card receivables-trend-card"><div class="section-heading"><div><h3>Last 3 Months</h3><p class="card-sub">Credit sales, immediate sales, and actual collections by month.</p></div></div><div class="receivables-trend-wrap"><canvas id="receivables-trend-chart"></canvas></div></section>
...
export function drawReceivablesTrend() {
  ...
  (window as any).__receivablesTrendChart = new ChartCtor(canvas, { type: 'bar', data: { labels, datasets: [
    { label: 'Credit Sales', data: receivablesTrend.map(row => row.credit_sales), backgroundColor: '#f28c28', borderRadius: 5 },
    { label: 'Immediate Sales', data: receivablesTrend.map(row => row.immediate_sales), backgroundColor: '#4da3d8', borderRadius: 5 },

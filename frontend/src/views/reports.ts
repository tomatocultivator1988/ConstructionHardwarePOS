...
export async function exportBalanceSheet() {
  showExportPeriodModal('Balance Sheet', async (period, format) => {
    ...
    const manual = data.manual_accounts || {};
    const manualVal = (k: string) => manual[k] !== undefined && manual[k] !== null ? Number(manual[k]) : null;
    const knownAssets = Number(assets.known_total || 0);
    const knownLiabilities = Number(liabilities.known_total || 0);
    const knownEquity = Number(equity.known_total ?? equity.retained_earnings ?? 0);
    const knownLiabilitiesAndEquity = knownLiabilities + knownEquity;
    const difference = knownAssets - knownLiabilitiesAndEquity;
    const rows: unknown[][] = [];
    ...
    line('Bank balance', manualVal('bank'), 'Manual admin account');
    line('GCash balance', manualVal('gcash'), 'Manual admin account');
    ...
    line('Land', manualVal('land'), 'Manual admin account');
    line('Equipment', manualVal('equipment'), 'Manual admin account');
    line('Building', manualVal('building'), 'Manual admin account');
    line('Other fixed assets', manualVal('other_fixed_assets'), 'Manual admin account');
    section('OTHER ASSETS');
    line('Trademark / intellectual property', manualVal('trademark'), 'Manual admin account');
    ...
    line('Mortgage payable', manualVal('mortgage_payable'), 'Manual admin account');
    line('Other current liabilities', manualVal('other_current_liabilities'), 'Manual admin account');
    section('LONG-TERM LIABILITIES');
    line('Long-term debt', manualVal('long_term_debt'), 'Manual admin account');
    line('Notes payable', manualVal('notes_payable'), 'Manual admin account');
    line('Other long-term liabilities', manualVal('other_long_term_liabilities'), 'Manual admin account');
    total('TOTAL LIABILITIES', knownLiabilities, 'Total recorded liabilities and payables');
    section("OWNER'S EQUITY");
    line("Owner's capital", manualVal('owner_capital'), 'Manual admin account');
    line('Retained earnings', equity.retained_earnings, 'Cumulative recorded sales less COGS and expenses');
    line("Owner withdrawals", manualVal('owner_withdrawals'), 'Manual admin account');
    total("TOTAL OWNER'S EQUITY", knownEquity, 'Owner capital + retained earnings less withdrawals');
    total("TOTAL LIABILITIES + OWNER'S EQUITY", knownLiabilitiesAndEquity, 'Total obligations and equity');
    total('CHECK / UNRECONCILED DIFFERENCE', difference, difference === 0 ? 'Balanced' : 'Difference between total assets and (liabilities + equity)');
...
export function showChartAccountModal(accountOrId: any = null) {
  const account = typeof accountOrId === 'string'
    ? (currentChartAccounts.find(a => String(a.id) === String(accountOrId)) || null)
    : accountOrId;
...
async function exportDetailedWorkbook(period: ExportPeriod) {
  ...
  const addBalanceSheet = (data: any) => {
    ...
    total("TOTAL OWNER'S EQUITY", knownEquity, 'Owner capital + retained earnings less withdrawals');
    total("TOTAL LIABILITIES + OWNER'S EQUITY", knownLiabilitiesAndEquity, 'Total obligations and equity');
    total('CHECK / UNRECONCILED DIFFERENCE', difference, difference === 0 ? 'Balanced' : 'Difference between total assets and (liabilities + equity)', true);

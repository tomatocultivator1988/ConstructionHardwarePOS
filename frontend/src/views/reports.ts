...
async function loadCashFlowReport(from?: string, to?: string) {
  ...
  return `<div class="report-section-heading"><div><h3>POS Cash Flow</h3><span>${fmtDate(start)} – ${fmtDate(end)} · Cash drawer activity only</span></div></div><div class="report-filters"><label>From</label><input id="rpt-cash-from" type="date" value="${start}" /><label>To</label><input id="rpt-cash-to" type="date" value="${end}" /><button class="btn btn-primary btn-sm" onclick="reloadCashFlow()">Load</button></div><div class="dashboard-grid report-metrics report-metrics-4"><div class="dashboard-card card-success"><div class="card-label">Cash Sales</div><div class="card-value">${fmtPeso(cash.cash_receipts)}</div></div><div class="dashboard-card card-danger"><div class="card-label">Cash Refunds</div><div class="card-value">${fmtPeso(cash.cash_refunds)}</div></div><div class="dashboard-card card-warning"><div class="card-label">Cash Expenses</div><div class="card-value">${fmtPeso(cash.cash_expenses)}</div></div><div class="dashboard-card card-info"><div class="card-label">Net Cash Movement</div><div class="card-value">${fmtPeso(cash.net_cash_change)}</div></div></div><div class="dashboard-grid report-metrics report-metrics-4" style="margin-top:var(--space-4)"><div class="dashboard-card"><div class="card-label">Cash In</div><div class="card-value">${fmtPeso(cash.cash_in)}</div></div><div class="dashboard-card"><div class="card-label">Cash Out</div><div class="card-value">${fmtPeso(cash.cash_out)}</div></div><div class="dashboard-card"><div class="card-label">Opening Cash</div><div class="card-value">${fmtPeso(cash.opening_cash)}</div></div><div class="dashboard-card"><div class="card-label">Closed Shifts</div><div class="card-value">${cash.shift_count || 0}</div></div></div><div class="chart-card" style="margin-top:var(--space-4);background:var(--c-surface);border:1px solid var(--c-border);border-radius:var(--radius-lg);padding:var(--space-5)"><div class="chart-title">Cashier Reconciliation</div><div class="summary-line"><span>Expected cash</span><b>${fmtPeso(cash.expected_cash)}</b></div><div class="summary-line"><span>Counted closing cash</span><b>${fmtPeso(cash.closing_cash)}</b></div><div class="summary-line total"><span>Drawer variance</span><b class="${Number(cash.variance) === 0 ? 'positive' : 'negative'}">${fmtPeso(cash.variance)}</b></div></div>`;
}
